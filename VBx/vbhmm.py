...
if __name__ == '__main__':
    ...
    for file_name, segs in recit:
        ...
        ######### save xvectors to cvs #########
        xvectors_path = os.path.join(args.out_rttm_dir, f'{file_name}_normalized_xvectors.csv')
        df = pd.DataFrame(index=seg_names, columns=list(range(x.shape[1])))
        df.loc[:] = x
        df.to_csv(xvectors_path)
        ########################################
